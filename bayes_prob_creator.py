...
for line in sys.stdin:
	line = line.strip()
	words = line.split("\t")
	if len(words) == 5: #then it is a line for a feature value
		if words[0] == "0": #if no-click
	...
	elif len(words) == 4: #then it is the total probability of either click or no-click
		if words[0] == "1":
			prob_click = words[2]
		elif words[0] == "0":
